...
Open the `Test Suites/main/TS1_demoaut.katalon.com`. You can just run it by clicking the usual image:${PAGES_URL}/images/run_katalon_test.png[button].
...
There are several tools to view HAR files effectively. I would recommend link:https://code.visualstudio.com/[Visual Studio Code] with link:https://marketplace.visualstudio.com/items?itemName=unclebeast.har-viewer[HAR Viewer]. The following images show an example:
...
The detail is wrapped in the `openBrowser` method of the custom class link:{REPOS_URL}/Keywords/com/kazurayam/ks/WebDriverPlusHARFactory.groovy[`com.kazurayam.ks.WebDriverPlusHARFactory`], which does the following:
...
- link:{PAGES_URL}/main_TS1_demoaut.katalon.com[`<projectDir>/work/main_TS1_demoaut.katalon.com/pretty.har`]
