...
- updated at Aug 2024, the tag 2.1.2 was confirmed runnable with Katalon Studio v8, does not run on KS v10.
- updated at April 2025, the tag 2.4.0 was confirmed runnable with Katalon Studio v10, no longer runnable with KS v8.

== Background

In the link:https://forum.katalon.com/[Katalon User Forum], there is a Frequently Asked Question.git
...
The post suggested a way to make use of BrowserMob Proxy in Katalon Studio. Unfortunately the sample code by matteo.lauria were incomplete (e.g, the `import` statements were trimmed off), so it was difficult for others to reproduce.

== What is this project does?
...
Additionally, this project would provides a few sample codes that post-processes a large HAR file to extract the request-response loggs that contain information of your interest.


== Procedure to run the demonstration project
...
3. You want to resolve external dependencies using Gradle build tool. In a Terminal window, execute the following bash command:
...
Open the `Test Suites/TS1_demoaut.katalon.com`. You can just run it.
...
When finished, you will find a new folder `work` has been created with 2 files: `pretty.har` and `extract.har`

image:{PAGES_URL}/images/TS1_result.png[]

These 2 files have very different size:
...
$ ls -la
total 26808
drwxr-xr-x@ 4 kazuakiurayama  staff       128  4月  1 22:07 .
drwxr-xr-x@ 3 kazuakiurayama  staff        96  4月  1 22:07 ..
-rw-r--r--@ 1 kazuakiurayama  staff     12001  4月  1 22:07 extract.har
-rw-r--r--@ 1 kazuakiurayama  staff  13711030  4月  1 22:07 pretty.har
----

The `pretty.har` has the size of 13 Megabytes, which contains the full content retured by BrowserMob Proxy. The `extract.har` has only 12 Kilobytes, which was generated by the TS1 while extracting the records out of the raw HAR content against a certain criteria.

== How to look into HAR files

The raw HAR file is very large. If you open the `work/main_TS1_demoaut.katalon.com/pretty.har` file using a text editor in Katalon Studio, you would certainly find it is too large to see. Text editor is not enough for HAR.

There are several tools to vew HAR. I would recommend link:https://code.visualstudio.com/[Visual Studio Code] with link:https://marketplace.visualstudio.com/items?itemName=unclebeast.har-viewer[HAR Viewer]. The following images show an example:
...
=== link:https://github.com/kazurayam/BrowserMobProxyInKatalonStudio/blob/develop/Scripts/main/common/startProxy_WebUI.openBrowser/Script1721103602049.groovy[Test Cases/main/common/startProxy_WebUI.openBrowser]
...
=== link:https://github.com/kazurayam/BrowserMobProxyInKatalonStudio/blob/develop/Scripts/main/interactions/visit_demoaut.katalon.com/Script1721514910231.groovy[Test Cases/main/interactions/visit_demoaut.katalon.com]
...
=== link:https://github.com/kazurayam/BrowserMobProxyInKatalonStudio/blob/develop/Scripts/main/common/closeBrowser_stopProxy/Script1673684270460.groovy[Test Cases/main/common/closeBrowser_stop]
...
The script pretty-printed (insert newlines, insert indentations) and saved into the `-pretty.har` file. The original 900MB is broken down into 4000 lines by link:https://github.com/kazurayam/JsonFlyweight[com.kazurayam.jsonflyweight.JsonFlyweight], a JSON pretty-printer library that I developped.


=== link:https://github.com/kazurayam/BrowserMobProxyInKatalonStudio/blob/develop/Scripts/main/post-process/select_entries_for_jquery_or_fontawesome/Script1721610779882.groovy[Test Cases/main/post-process/select_entries_for_jquery_or_fontawesome]
...
The script link:https://github.com/kazurayam/BrowserMobProxyInKatalonStudio/blob/develop/Scripts/main/post-process/select_entries_for_jquery_or_fontawesome/Script1721610779882.groovy[Test Cases/main/post-process/select_entries_for_jquery_or_fontawesome] does exactly what I wanted to do. I got a success.
...
I employed link:https://github.com/json-path/JsonPath[Jayway JsonPath] library. There are several articles about JsonPath on the net.

- link:https://www.baeldung.com/guide-to-jayway-jsonpath[Introduction to JsonPath, Baeldung] --- this article introduced me to JsonPath. It was enough for me.
- link:https://codippa.com/jayway-jsonpath-java/[Comprehensive Guide to JsonPath, CODDIPA]
...
- link:https://github.com/katalon-studio/katalon-gradle-plugin[Katalon Gradle Plugin]

To do this, you need to install Java and Gradle into your machine. The following post has an instruction how to:

- link:https://forum.katalon.com/t/automated-visual-inspection/81966#setting-up-gradle-build-tool-22[setting up gradle build tool]

Once you installed Java and Gradle, you want to write link:(https://github.com/kazurayam/BrowserMobProxyInKatalonStudio/blob/develop/build.gradle[`<projectDir>/build.gradle`] as follows:
