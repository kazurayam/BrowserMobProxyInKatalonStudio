...
When the command finished, you will obtain a set of jar files downloaded from the Maven Central repository on the Internet into the `Drivers` folder, as follows:
    ...
    ├── katalon_generated_jsonflyweight-0.1.5.jar
...
Open the `Test Suites/main/TS1_demoaut.katalon.com`. You can just run it by clicking the usual ![button]($https://kazurayam.github.io/BrowserMobProxyInKatalonStudio/images/run_katalon_test.png).

![TS1](https://kazurayam.github.io/BrowserMobProxyInKatalonStudio/images/TS1_demoaut.katalon.com.png)

You can choose a browser of Chrome, Chrome (headless), Forefox, Firefox (headless) or Edge Chromeium.

The Test Suite will run for about 1 minute.

When finished, you will find a new folder named `work/main_TS1_demoaut_katalon_com` containing 2 files: `pretty.har` and `extract.har`
...
The `pretty.har` contains raw HTTP payloads of HTTP Request and Response captured by BrowserMob Proxy during the Test Suite execution. It has the size of 13 Megabytes.

The `extract.har` has only 12 Kilobytes, which was generated by the Test Suite while extracting the several records out of the raw HAR content against a certain criteria.

### HAR Viewer tool

The raw HAR file is very large. Usually it contains thousands of lines. If you open the `work/main_TS1_demoaut.katalon.com/pretty.har` file using a text editor in Katalon Studio, you would certainly feel depressed to look into. A text editor is poor to view a HAR.

There are several tools to view HAR files effectively. I would recommend [Visual Studio Code](https://code.visualstudio.com/) with [HAR Viewer](https://marketplace.visualstudio.com/items?itemName=unclebeast.har-viewer). The following images show an example:
...
Let me show the scripts and describe a bit about them.
...
The detail is wrapped in the `openBrowser` method of the custom class [`com.kazurayam.ks.WebDriverPlusHARFactory`](https://github.com/kazurayam/BrowserMobProxyInKatalonStudio/tree/develop//Keywords/com/kazurayam/ks/WebDriverPlusHARFactory.groovy), which does the following:
...
The source of the custom class is concise. Bt it does a series of complicated processings using Groovy’s Meta-programming technique. Please read the code if you are interested in the detail.
...
2.  call the `closeBrowserExportHAR` method of the custom class `com.kazurayam.ks.WebDriverPlusHARFactory`, which does the following:
    ...
    - export a HAR instance out of the BrowserMob Proxy Server. Pretty-print the JSON as HAR and write it into the given file. Stop the proxy process.

This step will take just a few seconds to finish.

When this step finished, you will find 2 files created

- [`<projectDir>/work/main_TS1_demoaut.katalon.com/pretty.har`](https://kazurayam.github.io/BrowserMobProxyInKatalonStudio/main_TS1_demoaut.katalon.com)
...
How did I implement it? --- Please read the source code and find out. The most essential part is this:
...
This code fragment may look like a magic spell? --- Yes, it is very magical.

I employed [Jayway JsonPath](https://github.com/json-path/JsonPath) library. There are several articles about JsonPath on the net.
...
You want to get HAR in your own Katalon project? Do you want to find out how to? --- Well, please study this demonstration, reuse whatever code in the project as reference. The `build.gradle` file and the files in the `Keywords` folder are essential and reusable in your project.
